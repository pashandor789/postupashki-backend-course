...
### Дополнение по поводу использования rust для написания дз от доброжелателей
Для того, чтобы запустить тесты (с условием того, что вы инициировали новый проект cargo в папке с заданием), необходимо поместить файлы скриптов (client.rs, server.rs) в новую папку `./src/bin`, иначе cargo будет искать файл скрипта main.rs в папке `./src` и не сможет скомпилировать проект.
