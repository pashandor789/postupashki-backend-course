...
Напишите `swagger` документацию (для языка `golang` можете воспользоваться генерацией документации из комментариев - см. `swag-go`).  
...
Тесты написаны в файле `tests.py` с помощью модуля `pytest`. Напишите свой собственный скрипт для того, чтобы собирать ваш сервис в контейнере и тестировать его (рекомендую написать `Makefile`.
